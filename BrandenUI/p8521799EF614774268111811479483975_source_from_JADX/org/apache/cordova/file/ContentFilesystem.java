...
public class ContentFilesystem extends Filesystem {
    ...
    public JSONObject getEntryForLocalURL(LocalFilesystemURL inputURL) throws IOException {
        if ("/".equals(inputURL.fullPath)) {
            return Filesystem.makeEntryForURL(inputURL, Boolean.valueOf(true), inputURL.URL.toString());
        }
        ...
        if (cursor != null) {
            try {
                if (cursor.moveToFirst()) {
                    ...
                    return Filesystem.makeEntryForPath(inputURL.fullPath, inputURL.filesystemName, Boolean.valueOf(false), filePath);
                }
            } finally {
                if (cursor != null) {
                    cursor.close();
